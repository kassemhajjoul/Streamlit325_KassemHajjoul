...
rad=st.sidebar.radio("Navigation",["Home","Visualizations", "Tutorial"])

if rad== "Home":
    
    st.title("MSBA 325 Assignment 2")
    st.markdown("My name is Kassem Hajjoul, this home page will introduce you to the dataset and its table.")
    ...
    st.markdown("This is the full table of the dataset, it displays social anad economic factors affecting the happiness score in 150 countries.")
    st.markdown("Would you like to download the dataset?")
    if st.button("Download Dataset"):
        "https://docs.google.com/spreadsheets/d/e/2PACX-1vQ6LntwY7UX5EP4UikQn2-AfzzhaVmnAVYbR0Tj0gJZ_AHwvueSOhshraFIJeelYixzqHgK-8a3Ottk/pub?gid=1546798312&single=true&output=csv"
...
#Plotting the effect of GDP PER Capita, Freedom and Social support on the total score
    ...
    st.markdown("This scatter plot shows the relationships between the happiness score and the GDP per capital, Social support and freedom; These factors have positive effect on the happiness score.")
    st.plotly_chart(ax1)
    st.header("Stacked bar plot for various economic and social factors per countries; We can notice the synchronized levels of these factors in the different countries.")
    ax3=px.bar(df,x="Country",y=["Score","GDP_per_capita","Generosity","Corruption"])
    st.plotly_chart(ax3)
    st.markdown("This stacked bar plot displays the GDP per capital, generosity and corruption per country.")
#Top 10 countries
    ...
    st.plotly_chart(ax5)
    st.markdown("This histogram targets the healthy life expectancy of the countries.")
    x=df['Country']
    y=df['Score']
    z=df["Corruption"]
